...
public class ArticleService {
    ...
    public void createArticle(ArticleCreateRequest request, Long memberId, MultipartFile file) {
        String imageUrl = s3Service.upload(file);
        articleWriter.create(request, memberId, imageUrl);
    }
    ...
    public PageResponse<ArticleResponse> searchArticles(String keyword, String tag, Long categoryId, Pageable pageable) {
        ArticleSearchRequest request = new ArticleSearchRequest(keyword, tag, categoryId, null, pageable);
        return PageResponse.from(articleReader.search(request));
    }

    public PageResponse<ArticleResponse> searchMyArticles(String keyword, String tag, Long categoryId, Pageable pageable, Long memberId) {
        ArticleSearchRequest request = new ArticleSearchRequest(keyword, tag, categoryId, memberId, pageable);
        return PageResponse.from(articleReader.search(request));
    }
}
